...
## Update Log
- 2026-07-07: Footer に Last updated 表記追加（Footer.astro の `LAST_UPDATED` 定数、**経歴・スキル内容を見直した時に手動更新**。ビルド日時の自動挿入は内容鮮度と乖離するため不採用）
- 2026-06-11: main push での自動デプロイ追加（GitHub Actions + wrangler-action、Secrets 設定済み）。generate-blog-post.yml 削除（ブログ休止中・APIキー未設定でエラーが積み重なっていた）
